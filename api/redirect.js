export default async function handler(req, res) {
  try {
    ...
    // Log (await so the POST actually leaves before the function ends)
    ...
    if (LOG_ENDPOINT) {
      try {
        ...
        const logRes = await fetch(LOG_ENDPOINT, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        });
        if (!logRes.ok) {
          console.error("LOG_ENDPOINT non-200", logRes.status, await logRes.text());
        }
      } catch (err) {
        console.error("Logging fetch failed:", err?.message || err);
      }
    }

    // Pure 302 redirect, no body
